...
Before diving into the "why", let's address the "does it work?" question with actual data. We tested Claude Sonnet 4.5 on its ability to generate working Great Tables code from prompts.

Here's what happened in those first three examples:

![](GT-LLM-results-claude-sonnet-4-5.png)

We found that LLMs can generate Great Tables code reliably, but sometimes they need the right context. This isn't much a surprise really since it's how LLMs work with any specialized API. Though we do put a lot of effort making Great Tables LLM-friendly. We use composable methods, consistent naming, and declarative syntax and these practices happen to map more naturally with how you'd generally want to describe tables.

What this means is that if you're working with an LLM to create tables, providing API context (through documentation, examples, or tools like GitHub Copilot with access to the codebase and project site docs) improves your chances of getting working code on the first try. Also, if the LLM gets it wrong you can just point out the error (i.e., provide the error text itself) and the second response will probably run successfully. That said, it's amazing that an LLM can so frequently provide working code on the first try (and I've been observing it getting better over the last few updates of Claude Sonnet). 
