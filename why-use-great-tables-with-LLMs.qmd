...
Before diving into the "why", let's address the "does it work?" question with actual data. We tested three frontier models (Claude Sonnet 4.5, GPT-5, and Gemini 2.5 Pro) on their ability to generate working Great Tables code from natural language prompts.

The results were... instructive:

**Without API documentation (zero-shot)**: 17% success rate. Out of 15 attempts across 5 examples, only 4 produced working code. The rest crashed with hallucinated methods like `.tab_footnote()`, `.style()`, and invented parameters.

**With API documentation**: 73% success rate on the first 3 examples. The same prompts, but with access to an LLM-friendly API reference, increased success rates by 4.3x.

Here's what happened in those first three examples:

![](GT-LLM-results-claude-sonnet-4-5.png)

The API Latency example was straightforward: basic formatting and conditional styling. All three models got it right even without documentation. But as complexity increased (row grouping, column spanners, multiple formatting types), the wheels came off without proper API context.

We found that LLMs can generate Great Tables code reliably, but they need the right context. This isn't much a surprise really since it's how LLMs work with any specialized API. Though we do put a lot of effort making Great Tables LLM-friendly. We use composable methods, consistent naming, and declarative syntax and these practices happen to map more naturally with how you'd generally want to describe tables.

What this means is that if you're working with an LLM to create tables, providing API context (through documentation, examples, or tools like GitHub Copilot with access to the codebase) dramatically improves your chances of getting working code on the first try.
...
If you're building a workflow that relies on LLM-generated Great Tables code, you'll want to test your prompts systematically rather than manually checking each output. The [chatlas](https://posit-dev.github.io/chatlas/) Python package, combined with the [Inspect AI](https://inspect.ai-safety-institute.org.uk/) framework, provides a powerful way to create automated evaluations.

### Setting up eval-based testing

First, install chatlas with eval support:
...
1. **Collect a dataset** of test cases (prompts + expected behavior)
2. **Create a solver** from your chat instance using `.to_solver()`
3. **Define a scorer** to grade the outputs (e.g., does the generated code run without errors?)
4. **Run the evaluation** to get quantitative results

### Example: Testing Great Tables prompts

Let's create an eval that tests whether an LLM can generate working Great Tables code for different scenarios:
...
# Create a chat instance with Great Tables context
chat = ChatOpenAI(
    system_prompt="""You are a Python expert helping users create Great Tables.
    Generate working Python code using the great_tables library.
    Always import required modules and use correct method names."""
)

@task
def great_tables_eval():
    return Task(
        dataset=json_dataset("great_tables_test_cases.jsonl"),
...
Use `.export_eval()` to capture successful interactions as test cases:
...
# Have a successful interaction
chat.chat("""Create a Great Tables table from this data:
endpoint,method,requests
/api/users,GET,125043
/api/posts,GET,89234

Format requests with comma separators and add a title 'API Requests'.""")

# Export this as a test case
chat.export_eval(
    "great_tables_test_cases.jsonl",
    target="""Code should:
    1. Import GT from great_tables and use polars/pandas for data
    2. Use .fmt_number() with use_seps=True for requests column
    3. Use .tab_header() for the title
...
Once you have test cases, run them via the inspect CLI:

```bash
inspect eval great_tables_eval.py
```

Then view results interactively:

```bash
inspect view
```

This gives you quantitative metrics on:
- How many prompts produce working code
- Which types of requests fail most often
- How changes to your system prompt affect success rates

### Why this matters for Great Tables

Our testing showed that success rates vary dramatically based on:
- Complexity of the request (simple formatting: 100%, complex aggregation: 0%)
- Whether API docs are provided (4.3x improvement)
- How prompts are structured

With systematic evals, you can:
- **Test different system prompts** to find what works best
- **Measure improvements** when you add API context
- **Catch regressions** when LLM providers update their models
- **Build confidence** before deploying LLM-assisted workflows

If you're building a data app that generates tables, or creating a tool that helps users create Great Tables code, this eval framework lets you validate that your prompts are actually working reliably.
...
Just as you'd use plotnine or seaborn for visualizations in your notebook, we believe that Great Tables is the right choice for tables. And when you combine it with LLM assistance, you get the best of both worlds: conversational specification with code-level control.

The future of data analysis involves people and LLMs working together. Great Tables is built for exactly this collaboration and it gives you the structure and tools you need while remaining accessible enough for LLMs to generate valid code.
