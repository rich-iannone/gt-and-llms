...
Before diving into the *why*, let's address the *does it work?* question with actual data. We set out to find out if Great Tables works well with LLMs, we ran these experiments, and this post outlines what we found.

We systematically tested Claude Sonnet 4.5 on three progressively complex scenarios: (1) a simple table with basic formatting requirements, (2) a salary table with row grouping and currency formatting, and (3) a repository stats table with nanoplots and multi-level styling.
...
Much better! The table structure makes comparisons easier. But we're still seeing some limitations:
...
## Testing Your Prompts Systematically with chatlas and Inspect

If you're building a workflow that relies on LLM-generated Great Tables code, you'll want to test your prompts systematically rather than manually checking each output. The [chatlas](https://posit-dev.github.io/chatlas/) Python package provides a clean interface for interacting with LLMs programmatically, while [Inspect](https://inspect.aisi.org.uk) is an open-source framework for evaluating LLM applications (letting you define test cases, run them systematically, and score the results). Together they create a powerful way to build automated evaluations for code generation tasks.
...
With systematic evals, you can validate that documentation helps through quantifying exactly how much API context improves success rates. Also, you'll inevitably catch regressions by re-running tests when LLM providers update models. All of this builds confidence knowing your prompts work reliably before deploying workflows.

If you're building a data app that generates tables or you're creating tools that help users create Great Tables code, this eval framework lets you validate that your prompts actually work.
